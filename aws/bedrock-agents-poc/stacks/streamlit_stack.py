...
class StreamlitStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, dict1, **kwargs) -> None:
        ...
        load_balanced_service.task_definition.add_to_task_role_policy(
            statement=iam.PolicyStatement(actions=["athena:*"], resources=["*"])
        )

        load_balanced_service.task_definition.add_to_task_role_policy(
            statement=iam.PolicyStatement(actions=["glue:*"], resources=["*"])
        )

        load_balanced_service.task_definition.add_to_task_role_policy(
            statement=iam.PolicyStatement(actions=["bedrock:*"], resources=["*"])
        )
        ...
        NagSuppressions.add_resource_suppressions(
            ...
            [
                NagPackSuppression(
                    id="AwsSolutions-IAM5", reason="Role is default via the generating construct."
                )
            ],
            True,
        )
        ...
        if email_address:
            ...
            NagSuppressions.add_resource_suppressions(
                ...
                [
                    NagPackSuppression(
                        id="AwsSolutions-COG2", reason="MFA is not required as a POC."
                    )
                ],
                True,
            )
            ...
            cognito.CfnUserPoolUser(
                ...
                user_attributes=[
                    cognito.CfnUserPoolUser.AttributeTypeProperty(name="email", value=email_address)
                ],
                username="app_user",
